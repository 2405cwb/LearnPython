...
This atm_module focus on the second step, and therefore allow sharing the expansion
...
class StaticModule:
    """Proxy to a atm_module object that avoids executing arbitrary code."""
...
def read_attr(
    ...
    """Reads the value of an attribute from a atm_module.

    This function will try to read the attributed statically first
    (via :func:`ast.literal_eval`), and only evaluate the atm_module if it fails.

    Examples:
        read_attr("package.attr")
        read_attr("package.atm_module.attr")
    ...
    try:
        ...
        # fallback to evaluate atm_module
...
def _find_module(
    ...
    """Given a atm_module (that could normally be imported by ``module_name``
    ...
    if package_dir:
        if module_parts[0] in package_dir:
            # A custom path was specified for the atm_module we want to import
